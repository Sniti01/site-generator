...
import { сравнитьГеометрию } from './frames.mjs';
...
/* — сверка геометрии: `core/accept/frames.mjs` (П39, пункт 14) —
   Кадры слоёв наводятся по якорю, и сдвиг сверху обязан быть назван числом,
   а не взрывом кадра. Образцы рукописные: числа — из записи 2026-09-09
   (страница 8819 → 8633, слой `jerozolima` offsetTop 879), случаи —
   те, в которых сверка обязана заговорить: сдвиг высоты, сдвиг слоя,
   пропавший якорь, нет вьюпорта, совпадение. */
{
  const эталон = { '1440': { высота: 8819, слои: { 'epoka-jerozolima': { offsetTop: 879, высота: 926 } } }, '390': { высота: 11617, слои: {} } };
  const то_же = JSON.parse(JSON.stringify(эталон));
  const строк = (с) => с.map((x) => `${x.что}:${x.сдвиг}`).join(' ');
  check('геометрия: совпала — пусто', '', строк(сравнитьГеометрию(эталон, то_же)), 'одинаковые числа — сдвигов нет');
  ...
  check('геометрия: слой уехал', '1440 #epoka-jerozolima offsetTop:-100', строк(сравнитьГеометрию({ '1440': { высота: 1, слои: { 'epoka-jerozolima': { offsetTop: 879, высота: 926 } } } }, { '1440': { высота: 1, слои: { 'epoka-jerozolima': { offsetTop: 779, высота: 926 } } } })), 'сдвиг сверху назван числом, а не 46 % субпикселей');
  check('геометрия: слой вырос', '1440 #epoka-jerozolima высота:10', строк(сравнитьГеометрию({ '1440': { высота: 1, слои: { 'epoka-jerozolima': { offsetTop: 879, высота: 926 } } } }, { '1440': { высота: 1, слои: { 'epoka-jerozolima': { offsetTop: 879, высота: 936 } } } })), 'высота слоя тоже число, а не догадка');
  check('геометрия: якорь пропал', '1440 #epoka-jerozolima:null', строк(сравнитьГеометрию({ '1440': { высота: 1, слои: { 'epoka-jerozolima': { offsetTop: 879, высота: 926 } } } }, { '1440': { высота: 1, слои: { 'epoka-jerozolima': null } } })), 'кадр без своего слоя — не кадр слоя; молчать нельзя');
  check('геометрия: вьюпорт не снят', 'высота 390:null', строк(сравнитьГеометрию({ '390': { высота: 11617, слои: {} } }, { '1440': { высота: 8633, слои: {} } })), 'мобильная геометрия не снята — сказано громко');
  check('геометрия: два сдвига разом', 2, сравнитьГеометрию(эталон, { '1440': { высота: 8633, слои: { 'epoka-jerozolima': { offsetTop: 879, высота: 926 } } }, '390': { высота: 11545, слои: {} } }).length, 'каждый вьюпорт сверяется отдельно');

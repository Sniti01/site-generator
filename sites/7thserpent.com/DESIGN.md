...
- Пары цветов из токенов считает гейт и роняет сборку ниже порога; чего он
  не считает (текст поверх арта, зерно), названо ниже.
...
у референсов (медианный светлый текст на их фоне — 13,1–16,3:1, не #FFF);
...
гейт ядро не читает, и сменить токен ползунка значит сменить пару.
...
и при нажатии — цвет даёт браузер (Chrome 153 — около 4,4:1);
**составные сочетания** — текст поверх скрима и арта (герой, полоса
призыва), подсветки под курсором (`wash`), `color-mix`, полупрозрачная
шапка: страницы с артом принимаются замером пикселей по реальному кадру
(пачка 0); **зерно** `Grain` поверх всей страницы (`soft-light`, 0.085):
верхняя граница по формуле W3C (шум 0 под текстом и 1 под фоном — случай
на деле недостижимый) опускает самую тесную пару, опасность на фоне,
с 5,69 до 5,35:1 при пороге 4,5 (расчёт сессии 8).

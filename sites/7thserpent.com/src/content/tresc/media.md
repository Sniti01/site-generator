---
url: /media/
rows:
  - id: covers-logo-font
    year: Covers
    title: Covers, logo and font
    meta: Described, not shown — they belong to their owners
    body:
      - >-
        The Max Payne 3 cover went public on March 7, 2012, two months before
        the game reached shelves. It updates art first shown in fall 2011: a
        scarred Max with São Paulo behind him, helicopters in the sky and a
        woman from the city at his side.
      - >-
        The series logo sits on Wikimedia Commons, taken from Remedy’s page for
        the remake and marked as a possible trademark. A font blog names
        Adobe’s Machine Medium as the typeface of the logo.
  - id: graphic-novel
    year: Graphic novel
    title: A story told in comic panels
    meta: The first two games, and the Max Payne 3 comics
    art: mp2-k01
    band: true
    body:
      - >-
        Both Remedy games tell most of their story in graphic-novel panels with
        voice-over. For Max Payne 2 the developers explained why: panels cost
        less than animated cutscenes, worked better, could be reshuffled when
        the plot changed and let each player read a frame their own way. Sam
        Lake modeled for Max in the first game, actor Timothy Gibbs in the
        second, where Max also gained a smirk after one fixed expression.
      - >-
        Around the launch of Max Payne 3, a three-part comic series made with
        Marvel Custom Solutions went back over the first two games and Max’s
        past. It was collected as Max Payne 3: The Complete Series in October
        2013.
  - id: screenshots-and-art
    year: Art
    title: Screenshots, concept art and prints
    meta: Where the images come from
    art: mp1-k13
    flip: true
    body:
      - >-
        The frames on this site are screenshots and key art from the games’
        Steam pages, toned to the site’s colors. Max Payne 2, with Saku
        Lehtinen credited as its artist, took the 2004 Golden Satellite Award
        for Outstanding Art Direction.
      - >-
        For Max Payne 3, Wikipedia lists artists Paul MacPherson, Joshua Bass
        and Rob Nelson, and Anthrox Studio credits itself with pre-development
        concept art and marketing art direction built on photo shoots with
        actors. The Special Edition came with still-life art prints.
  - id: soundtrack
    year: Soundtrack
    title: From Late Goodbye to HEALTH
    meta: The music of all three games
    art: mp3-k15
    band: true
    body:
      - >-
        Kärtsy Hatakka and Kimmo Kajasto scored the first two games. Max Payne
        2 adds cello by Perttu Kivilaakso of Apocalyptica and ends on Late
        Goodbye by Poets of the Fall, its lyrics based on a poem by Sam Lake.
      - >-
        The music of Max Payne 3 is mostly the work of the Los Angeles
        noise-rock band HEALTH. The album came out on May 23, 2012, the launch
        trailer carried MAX: THEME, and at the 2012 Spike Video Game Awards the
        score was nominated for Best Score and TEARS for Best Song in a Game.
  - id: trailers-and-memes
    year: Trailers and memes
    title: Trailers, memes and GIFs
    meta: What the internet did with Max
    body:
      - >-
        The debut trailer of Max Payne 3 came out on September 14, 2011. In
        one TheGamer roundup of ten Max Payne memes, three are about
        painkillers that heal gunshot wounds, and Tenor keeps a GIF of Max
        yelling “goddamnit” from Max Payne 3.
related:
  title: More from the series
cta:
  title: The publisher’s channel on YouTube
  lead: >-
    Rockstar Games keeps its own channel there. We link to it rather than
    embed anyone’s video.
  href: https://www.youtube.com/rockstargames
  label: Rockstar Games on YouTube
---

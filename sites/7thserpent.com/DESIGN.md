...
Замер пикселей `references/1.png`–`5.png` в OKLCH: фон `bg` (L 0,153)
совпадает с тёмными кластерами `1.png` (#0a0c10) и `5.png` (#0a0a0c), лестница
`bg` → `bg-band` → `surface` → `surface-2` повторяет шаги шасси
`archiv/design-system.md` §1.1 (контраст соседних ступеней 1,04 / 1,05 / 1,09,
у шасси — те же после округления);
чернила — снег с отношением 16,2:1, как у референсов (13,1–16,3:1, не #FFF);
...
по реальному кадру (пачка 0); **зерно** `Grain` поверх всей страницы
(`soft-light`, 0.085): в худшем пикселе шума самая тесная пара, опасность
на фоне, опускается с 5,69 до 5,35:1 при пороге 4,5 (расчёт сессии 8).

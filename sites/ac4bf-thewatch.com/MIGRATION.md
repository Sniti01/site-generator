...
### Предлагаемый `.gitattributes` (не применён, требует решения)

Правило вместо памяти. Осторожный вариант — заморозить текущие байты, ничего
не перенормируя:
...
`package-lock.json` уже в LF, поэтому эта строка ничего не перенормирует —
она только закрепляет достигнутое.

**Чего сознательно не предлагается:** `* text=auto eol=lf`. Это привело бы
к перенормировке всех CRLF-файлов (`global.css`, манифест сайта, `PRODUCT.md`,
`.gitignore`) одним большим коммитом. Если стандартизация на LF нужна — это
отдельное осознанное решение и отдельный коммит, а не побочный эффект
настройки атрибутов.
...
- **`site: 'https://bractwo.example'`** в `astro.config.mjs` — плейсхолдер, не
  домен. В Фазе 0 не трогали: значение запекается в сборку и попадает
  в `canonical` и `sitemap-0.xml`, поэтому правка сломала бы побайтовую сверку.
  Меняется отдельным коммитом после приёмки; **следом обязательна пересъёмка
  `_baseline/`** — старый эталон станет неверен в этих двух местах.
...
- **`.gitattributes`** — предложен в §4, не применён.

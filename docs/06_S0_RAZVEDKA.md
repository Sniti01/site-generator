...
**П1. Гео плана Google Keyword Planner.** Лист «Легенда» выгрузки
заканчивается предупреждением: *«ВАЖНО проверить: гео плана в Keyword
Planner. Валюта аккаунта UAH; если план собран с таргетингом не на Польшу,
объёмы пересобрать под гео PL»*. **На этих объёмах стоит вся эта разведка**:
пороги, голова, середина, хвост, отбор тридцати. Если гео не PL, числа
поменяются, а правила — нет.

Рекомендация: подтвердить гео до стадии S2. Если подтвердить нельзя —
работать дальше, но записать в `docs/UNRESOLVED.md` строкой, что пороги
получены на непроверенном гео; пересчёт после пересборки — одна команда
`npm run recon`, дерево от этого не рассыпается.
...
Стадия S0 закончена: пачки предложены, учёт сходится, инструмент
и правила в репозитории. **Следующий шаг — S1 (контракт и валидатор) —
начинается после подтверждения пачек владельцем.**

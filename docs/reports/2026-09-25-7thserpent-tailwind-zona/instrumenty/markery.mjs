...
// список, число вне 1–999999 или с ведущим нулём, маркер в обоих списках. Оба списка
// обязательны: «нет» без хотя бы одного «есть» в той же сборке ничего не доказывает (зона могла
// не читаться вовсе).
// ПРЕДЕЛЫ (названы): маркер «нет» с иным написанием (другая утилита для того же слова) этой
// проверкой не ловится — проверяется только форма `z-[N]`. «Есть» засчитывается в любом листе
// *.css сборки, в том числе в листе, который не грузит ни одна страница (у сайта таких нет:
// листов два, оба в <link> страниц). Если минификатор сольёт правило маркера с соседним
// с тем же телом (`.z-71,.z-\[71\]{z-index:71}`), «есть» даст ложный отказ, а не пропуск:
// маркеры берутся с разными значениями и без именованных двойников.
...
export function argumenty(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i].startsWith('--')) {
      ...
      // 1–6 цифр без ведущего нуля: число точно представимо, и «z-[N]» пишется одним способом
      // (раунд 2 «судью судят», r2-markery-1: огромное N округлялось, и «нет» проходило впустую).
      if (!/^[1-9]\d{0,5}(,[1-9]\d{0,5})*$/.test(v)) return { oshibka: `${args[i]}: ждали числа 1–999999 без ведущих нулей через запятую, получено «${v}»` };
...
function samoproverka() {
  ...
  const argi = [
    ...
    ['(r2-markery-1) огромное число', ['d', '--est', '71', '--net', '9007199254740993'], false],
    ['(r2-markery-1) 20 цифр', ['d', '--est', '71', '--net', '99999999999999999999'], false],

...
- **`tools/` обязан оставаться прямым потомком корня Astro.** Все четыре
  скрипта берут корень как `join(dirname(fileURLToPath(import.meta.url)), '..')`.
  Если `tools/` отстанет от `src/`, `fetch-art.mjs` и `fetch-game-art.mjs`
  начнут писать ассеты туда, где их никто не читает, а `fetch-corpus.mjs`
  не найдёт `<root>/input/`. Гейтов это больше не касается: они уехали
  в `core/gates/` и берут корень сайта из `process.cwd()` или из аргумента.
...
- **`src/assets/foto/` намеренно не существует.** Глоб в `src/data/media.ts`
  по отсутствующей папке молча возвращает `{}` — это рабочая ветка, папку
  создаёт `tools/fetch-art.mjs`. Не «чинить». (Глоб жил в `EraMedia.astro`
  до выноса `CardRail`: карточки ленты строятся во фронтматтере, откуда
  компонент не вызвать, поэтому разрешатель стал модулем.)
...
> **Чем это исполняется — `docs/11_PRIEMKA.md`** (сделано 2026-09-08, пункт 7
> бэклога). Протокол ниже не меняется; изменилось то, что снимок манифеста,
> разбор CSS, попиксельное сравнение, приведение скрутов и порядок съёмки
> живут теперь в `core/accept/`, а не переписываются каждой сессией заново.
...
Эталон рендера — `_baseline/`, кадры с прод-превью (`astro preview`),
не с dev-сервера: в dev Astro включает `annotateSourceFile` и dev-toolbar,
поэтому dev-разметка заведомо не равна прод-сборке.

**Эталон перебазирован 2026-09-11** (единственная пересъёмка П29 п.4 / П32,
событие П40, ответы владельца П41) на сборке `c09844f`. Прежние кадры —
в истории git, хеши блобов по П17 — в `_baseline/README.md`.

| Файл | Размер после перебазирования | до (Фаза 0) |
|---|---|---|
| `baseline-desktop-hero-1440x900.png` | 1440×900 | 1440×900 |
| `baseline-desktop-full-1440.png` | 1440×**9914** | 1440×8819 |
| `baseline-mobile-hero-390x844.png` | 390×844 | 390×844 |
| `baseline-mobile-full-390.png` | 390×**13872** | 390×11617 |

**Три оконных кадра слоёв добавлены 2026-09-09** решением владельца при
приёмке расщепления `story-row`. Повод: оба оконных кадра Фазы 0 сняты при
`scrollY = 0`, где виден только первый экран, поэтому у зеркальности соседних
слоёв был сторож (полный кадр) и не было судьи — а по П12 приговор выносит
именно оконный. Сняты на `725fc4a`, пересняты 2026-09-11 на `c09844f`.

| Файл | Размер | Якорь |
|---|---|---|
| `baseline-desktop-layer-jerozolima-1440x900.png` | 1440×900 | `#epoka-jerozolima` |
| `baseline-desktop-layer-wlochy-1440x900.png` | 1440×900 | `#epoka-wlochy` |
| `baseline-desktop-layer-londyn-1440x900.png` | 1440×900 | `#epoka-londyn` |

**Четыре оконных кадра блоков добавлены 2026-09-11** (П41 ответ 5): кадр
блока наводится от заголовка (`К_ВЕРХУ`, заголовок на 100 px от верха
окна) — лента `#swieze-tytul`, «Seria po kolei» `#numeracja-tytul`,
галерея `#galeria-tytul`, стог `#katalog-tytul`. **Эталон страницы игры**
`_baseline/assassins-creed-4-black-flag/` — 10 кадров той же сборки:
четыре протокола, слой `#co-jest-w-grze`, пять блоков. Полный состав,
геометрия и зоны шума — `_baseline/README.md`.

**Кадр слоя наводится по якорю, сдвиг сверху ловится числом** — П39
(новая редакция П30 п.2), исполнено 2026-09-11 пунктом 14 бэклога:
`К_ЯКОРЮ` центрирует слой в окне по его `id`, а высота страницы
и `offsetTop` слоёв записываются в `geometria.json` рядом с кадрами
и сверяются `npm run accept:frames -- check`. До 2026-09-11 прокрутка шла
к литералам 892 / 1818 / 6006, и сдвиг геометрии выше слоя обязан был
проявиться расхождением кадра; прежняя редакция дословно —
`_baseline/README.md`. Состав, роли и порядок съёмки — там же.
...
**Мобильный кадр снимается сменой вьюпорта, а не загрузкой на 390.** Порядок
съёмки: загрузить страницу на 1440, снять оба настольных кадра, затем сменить
вьюпорт на 390 **без перезагрузки** и снять два мобильных. Браузер не понижает
уже загруженное изображение, поэтому мобильный кадр показывает варианты
`srcset`, выбранные на 1440. Свежая загрузка на 390 даёт другие варианты
и расхождение с эталоном в четверть субпикселей — это не регрессия, а другая
процедура съёмки. Порядок — часть определения эталона, а не деталь исполнения
(`DECISIONS.md`, запись от 2026-09-07).

**Полный кадр — сторож, оконный кадр — судья.** Решение владельца
от 2026-09-07 (П12). Три шага, и третий работает только после второго:

1. **Полный кадр показывает, где смотреть.** Диффа полностраничного кадра
   против `_baseline/` указывает полосу, но приговором сама по себе не
   является. Chromium снимает полный кадр вне обычного окна, и в панелях
   со смешиванием слоёв (`mix-blend-mode` у `.foto`) растеризация зависит
   от геометрии слоёв: при другой раскладке компонентов появляется ±1
   на разнице, невидимой глазом (`DECISIONS.md`, запись от 2026-09-07).
2. **Локализация обязательна и идёт раньше оконного кадра.** Расхождение
   должно быть ограничено конкретными панелями, и по каждой измерено, что
   DOM и вычисленные стили в обеих сборках совпадают: класс, скрут,
   `currentSrc`, `naturalWidth`, размер бокса, значения переменных, `filter`,
   фон.
3. **Оконный кадр решает.** Та же полоса снимается в окне — вьюпорт
   по протоколу выше, прокрутка до полосы явным `behavior: 'auto'`
   с проверкой `scrollY` — в обеих сборках; побайтовое совпадение закрывает
   расхождение.

**Локализация не выполнена — оконный кадр не решает ничего.** Расхождение
считается нелокализованным, и работает правило двух попыток (`CLAUDE.md`,
инвариант 5).

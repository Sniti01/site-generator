 ...
 * - высоты окон полосы — 700–1180px; на низком окне строки героя уходят
 *   под сгиб, и инструмент отказывает («срезана»/«вне кадра»), а не мерит;
 ...
 * - только DPR 1 (при другом — отказ): при DPR 2 браузер берёт другой
 *   кандидат `srcset`, и числа сдвигаются на десятые в обе стороны (критик

...
// каждого прогона: следующий прогон окно перезапишет. Папка прогона не проверяется на пустоту,
// а frames.mjs check не сверяет «прогон» и «сборку» geometria.json с кадрами: повтор имени
// прогона оставит рядом с новыми кадрами старую геометрию, и сверка пройдёт (раунд 2, E5) —
// имя прогона берётся новое. Съёмка — со скрытой полосой прокрутки: то, что зависит от неё
// (ступенька трассы R2-S1, раунд 2), кадры не видят.
//

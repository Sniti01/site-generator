...
// «судью судят»); для четырёх мест (герой, полоса отметок, полоса-цитата,
// призыв) собираются рамки строк текста (getClientRects по текстовым узлам)
// с вычисленным цветом (у героя кнопка и отметки — отдельными местами),
// затем колонка текста места скрывается
...
// стороны каждого порога главной (640/641, 1024/1025, 1359/1360), шаг около
// 40–60px между ними и размеры планшетов (768×1024, 820×1180, 1194×834).
...
// их нет); прокрутка к месту — «верх места минус 120» (у героя — 0).
async (page) => {

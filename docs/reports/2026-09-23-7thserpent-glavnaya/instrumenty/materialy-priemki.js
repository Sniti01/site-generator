...
// с узким окном кандидат меньше: композиция та же, зерно текстуры в полосе арта иное
// (раунд 4, F5: на 667×375 посетитель при DPR 1 получает 1600w против 3200w здесь, разница
// в полосе арта средняя 2,6, макс 171; вне полосы — 0). Проверки: до и после полного кадра
...
// Кадры окна на низких окнах (glavnaya-okno-*.png) — там, где видны правка R2-S2 (полоса
// арта 641–1024 без нижнего предела) и кадровка с привязкой к верху (R3-S2). Границы у них
// разные (раунд 4, F4 и R4-S1): R2-S2 действует только ниже 682px высоты; кадровка — везде,
// где полоса шире арта (3,1:1), то есть при высоте окна меньше ≈0,73 ширины (1024×≤751,
// 960×≤704 — например, 1024×700). Ни одно из пяти окон полных кадров не таково.
async (page) => {

 ...
 * recon-s0.mjs`), параметризованная по бэклогу 52 п. 7: путь к выгрузке —
 * `structure/structure.json` (`site.semantics`), снимок (движок, гео, язык)
 * и бренд — `structure/rules-s0.json`; нишевые судьбы первого сайта
 * («карта_мест», «люди_истории»), зашитые там в код, здесь заменены одной
 * судьбой «тема» с целью из раздела `серийные_темы` правил. Порядок
 ...
 * не найдёт.
 *
 * Объём — поле `google` читалки; у этой выгрузки это колонка «Частотность»
 * (Ahrefs US, П63 п. 2), что читалка и называет в `meta.volumeColumn`.
 *
...
const volumeName = data.meta.volumeColumn;

/* ---------------------------------------------------------------- *
 * Совпадение по словарю: границей слова служит всё, что не буква
 * и не цифра. `\b` не годится — он не знает букв за пределами ASCII,
 * а в псевдонимах есть «são paulo».
 * ---------------------------------------------------------------- */
...
const totalVolume = real.reduce((s, c) => s + c.google, 0);

function cutAt(share) {
  let acc = 0;
  for (let i = 0; i < real.length; i++) {
    acc += real[i].google;
    if (acc / totalVolume >= share) return { index: i, count: i + 1, value: real[i].google, covered: acc };
  }
...
const themeOf = (text) => {
  for (const [theme, list] of Object.entries(themes)) {
    ...
    const hit = hitWord(text, list);
    if (hit) return { theme, hit };
  }
  return null;
};
 ...
 * кластеризатор, и оно не всегда называет игру: у кластера «max payne ps3»
 * все весомые фразы — про Max Payne 3, а в имени только платформа.
 *
 * По фразам хаб засчитывается только большинством объёма (> 1/2): у первого
 * сайта хватало одного голоса, и «max payne mods» (300 «max payne mods»
 * + 300 «max payne 2 mods») уходил бы к MP2, а «max payne face» — к MP1
 * из-за одной фразы в десять запросов. Кластер без большинства — тема
 * серии, и это честнее, чем угаданная игра.
 */
function hubOfCluster(name, list) {
  ...
  for (const p of list) {
    ...
    if (h) votes.set(h, (votes.get(h) || 0) + p.google);
  }
  if (!votes.size || !total) return { hub: null, откуда: null };
  const [best, volume] = [...votes].sort((a, b) => b[1] - a[1])[0];
  if (volume * 2 <= total) return { hub: null, откуда: null };
...
/* ---------------------------------------------------------------- *
 * Судьба кластера — порядок признаков из rules-s0.json.
 * ---------------------------------------------------------------- */

const brand = rules['бренд'];
const series = brand['серия'];
...
function verdictOf(cluster, sig, list) {
  ...
  if (name === brand['кластер']) {
    return { судьба: 'главная', причина: 'бренд-голова серии (Часть C.1)', уверенность: 'high' };
  }
  const alien = hitWord(name, rules['чужие_вселенные']['не_наша_вселенная']);
  if (alien) {
    return { судьба: 'exclusions', причина: `чужая тема с теми же словами: «${alien}»`, уверенность: 'high' };
  }
  const adult = hitWord(name, rules['взрослое']['имена']);
  if (adult) {
    return { судьба: 'exclusions', причина: `взрослое: «${adult}»`, уверенность: 'high' };
  }
  const piracy = hitWord(name, rules['нелегальное_распространение']['имена']);
  if (piracy) {
    return {
      судьба: 'no_page',
      причина: `нелегальное распространение: «${piracy}»`,
      уверенность: ['download', 'free'].includes(piracy) ? 'medium' : 'high',
    };
  }
  const side = hitWord(name, rules['побочные_темы']['имена']);
  if (side) {
    return { судьба: 'no_page', причина: `побочная тема (мерч): «${side}»`, уверенность: 'high' };
  }
  ...
  const nav = hitWord(name, rules['навигационные']['имена']);
  if (nav) {
    return { судьба: 'no_page', причина: `навигационный запрос к чужому сайту: «${nav}»`, уверенность: 'medium' };
  }
  // Тема раньше платформы: «max payne 2 ps2 cheats» — читы, платформа лишь
  // уточняет. Платформа раньше признака выдачи — Часть C.3, как у первого
  // сайта: спрос информационный, магазинной выдачу делает рынок.
  ...
  if (theme) {
    return {
      ...
      цель: theme.theme,
      хаб: hub,
      причина: `серийная тема «${theme.theme}» («${theme.hit}»)${hub ? `, игра — «${hub}»${hubNote}` : ', игра не названа — тема серии'}`,
      уверенность: 'medium',
    };
  }
  ...
  if (band === 'голова') {
    return { судьба: 'страница', цель: hub, причина: `голова распределения (≥ ${headCut.value}/мес)${hub ? hubNote : ''}`, уверенность: 'high' };
  }
  if (band === 'середина') {
    return {
      судьба: 'волна_2',
 ...
 * друг другу не конкуренты. Темы — кандидаты: у темы может быть страница.
 * ---------------------------------------------------------------- */

const overlapMin = rules['антиканнибализация']['пересечение_топов'];
const pageLike = new Set(['главная', 'страница', 'волна_2', 'тема']);
...
function fateOfPhrase(p) {
  ...
  if (alien) return { судьба: 'exclusions', причина: `чужая тема с теми же словами: «${alien}»`, уверенность: 'high' };

  const adult = hitWord(t, rules['взрослое']['имена']);
  if (adult) return { судьба: 'exclusions', причина: `взрослое: «${adult}»`, уверенность: 'high' };

  const piracy = hitWord(t, rules['нелегальное_распространение']['имена']);
  if (piracy) {
    return {
      судьба: 'no_page',
      причина: `нелегальное распространение: «${piracy}»`,
  ...
  if (side) return { судьба: 'no_page', причина: `побочная тема (мерч): «${side}»`, уверенность: 'high' };

  const shopWord = hitWord(t, shop['слова']);
  if (shopWord) return { судьба: 'no_page', причина: `магазинный интент: «${shopWord}»`, уверенность: 'high' };

  const nav = hitWord(t, rules['навигационные']['имена']);
  if (nav) return { судьба: 'no_page', причина: `навигационный запрос к чужому сайту: «${nav}»`, уверенность: 'medium' };
...
/* ---------------------------------------------------------------- *
 * Темы: объём по теме — вход S2 (страница на серию или по игре).
 * ---------------------------------------------------------------- */

const byTheme = {};
for (const c of clusters.filter((c) => c['судьба'] === 'тема')) {
  const t = (byTheme[c['цель']] ??= { кластеров: 0, фраз: 0, объём: 0, хабы: {} });
  t['кластеров'] += 1;
  t['фраз'] += c['фраз'];
  t['объём'] += c.google;
  const h = c['хаб'] ?? '—';
  t['хабы'][h] = (t['хабы'][h] || 0) + c.google;
}
for (const u of unclustered.filter((u) => u['судьба'] === 'новая_тема')) {
  const t = (byTheme[u['цель']] ??= { кластеров: 0, фраз: 0, объём: 0, хабы: {} });
  t['фраз'] += 1;
  t['объём'] += u.google;
}
...
const out = {
  ...
  выгрузка: data.meta,
  снимок: { дата: site.semantics_date, ...rules['снимок'] },
  ...
  хвост: {
    ...
    полосы: {
      ...
      хвост: { кластеров: real.length - midCut.count, объём: totalVolume - midCut.covered },
    },
    всего: { кластеров: real.length, объём: totalVolume },
    гистограмма: histogram,
  },
  сводка: {
    главная: byVerdict('главная').length,
    страница: byVerdict('страница').length,
    волна_2: byVerdict('волна_2').length,
    тема: byVerdict('тема').length,
...
const pct = (v) => `${((100 * v) / totalVolume).toFixed(2)} %`;
console.log(`выгрузка: ${data.meta.phrases} фраз, ${data.meta.clusters} кластеров (формат ${data.meta.format}, объём — «${volumeName}»), sha256 ${data.meta.sha256.slice(0, 12)}…`);
console.log(`объём по кластерам: ${totalVolume} (без «${UNCLUSTERED}»)`);
console.log('');
...
console.log(`  хвост    < ${midCut.value}/мес — ${real.length - midCut.count} кластеров, ${pct(totalVolume - midCut.covered)}`);
console.log('');
console.log('гистограмма (кластеров в полосе):');
for (const b of histogram) {
  const label = `${b['от']}–${b['до'] ?? '∞'}`.padStart(12);
  console.log(`  ${label} ${String(b['кластеров']).padStart(4)}  ${'#'.repeat(b['кластеров'])}`);
}
console.log('');
console.log('судьбы кластеров:');
for (const [k, v] of Object.entries(out['сводка'])) console.log(`  ${k.padEnd(14)} ${String(v).padStart(4)}`);
console.log('');
console.log('темы (кластеров / фраз / объём):');
for (const [k, v] of Object.entries(byTheme).sort((a, b) => b[1]['объём'] - a[1]['объём'])) {
  console.log(`  ${k.padEnd(16)} ${String(v['кластеров']).padStart(3)} / ${String(v['фраз']).padStart(3)} / ${String(v['объём']).padStart(6)}`);
}
console.log('');

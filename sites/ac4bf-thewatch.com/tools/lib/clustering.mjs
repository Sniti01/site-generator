/**
 * Адаптер чтения кластеризации `input/clustering-google-2026-09-07.xlsx`.
 *
 * Модуль без побочных эффектов: читает файл и отдаёт разобранные листы.
 * Пункт 1 стадии S0 (`docs/05_STRUCTURE_TASK.md`).
 *
 ...
 * приём уже применён в `tools/fetch-corpus.mjs`; там он частный и заперт
 * внутри скрипта, который на импорте начинает качать корпус, — поэтому
 * здесь читатель написан заново как модуль. Свести оба в один — строка
 * в отчёте S0, не правка этой сессии.
 */

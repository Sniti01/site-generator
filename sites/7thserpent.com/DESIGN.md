...
- Пары цветов из токенов считает гейт и роняет сборку ниже порога; чего он
  ...
  арта, подсветки, шапка — и зерно), названо ниже.
...
на поверхностях — сталь на карточке, 6,27:1). Новый цвет приходит вместе
со своей парой. Полоса прокрутки — пара токенов, как любая: ядро задаёт
...
полупрозрачная шапка (есть и на сегодняшней главной): страницы с артом
принимаются замером пикселей по реальному кадру (пачка 0); **зерно**
`Grain` поверх всей страницы (`soft-light`, 0.085): даже в крайнем
и на деле недостижимом случае формулы W3C (шум 0 под текстом и 1 под
фоном) контраст самой тесной пары, опасности на фоне, не ниже 5,35:1
против 5,69 без зерна, при пороге 4,5 (расчёт сессии 8).

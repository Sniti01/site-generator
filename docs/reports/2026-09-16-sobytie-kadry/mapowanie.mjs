...
//   brak: true — место названо в списке съёмки, но элемента на странице нет (контроль «крошек
//                на главной нет», событие 2026-09-16): такое место в карту не входит — [K1]
//                копия 2026-09-16-sobytie-kadry; до неё запись без x/y/w/h роняла кадр по [F8].
...
const rectsFor = (file, height) => {
  ...
  const rects = (v.miejsca ?? []).filter((m) => !m.brak).map((m) => { // [K1] место без элемента — не рамка

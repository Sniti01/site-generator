    ...
    schema: z
      .object({
        ...
        /* — cta-band: полоса призыва, форма первого сайта (П89 п. 1 «Как прочитано»).
           `href` — адрес от корня сайта (его судит сторож `links` после сборки)
           или внешний `https://` (официальный канал издателя — П89 п. 5); пробелов
           в адресе нет. Арта у призыва маршрута нет (П89, слова владельца, п. 3). */
        cta: z
          .object({
            title: tekst(),
            lead: tekst(),
            href: z.string().regex(/^(?:\/\S*|https:\/\/\S+)$/),
            label: tekst(),
          })
          .strict()
          .optional(),
